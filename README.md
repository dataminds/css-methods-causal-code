...
이 저장소의 **모든 내용**은 **CC BY-NC 4.0**(저작자표시·비영리)을 따릅니다. 데이터·생성기·배터리·노트북·낱장 전부입니다. 자세한 조건은 `LICENSE` 파일에 있습니다.

- **할 수 있는 것**: 강의·연구·학습 등 비영리 목적의 복제·배포·수정·2차 저작.
- **조건**: 출처 표시. 권장 표기 = 안도현 (2026). 『사회과학 통계와 인과추론』 실습 자료. https://github.com/dataminds/css-methods-causal-code (CC BY-NC 4.0)
- **영리 이용**: 별도 허락이 필요합니다(contact@minds.kr).
- **책 본문은 포함되지 않습니다.** 저작권자가 권리를 보유합니다. 다만 `worksheets/`의 두 낱장(부록 B 검증 로그 양식 · 부록 D 아홉 단계 체크리스트)은 **독자가 실제 연구에 들고 가 쓰라고 만든 양식**이라 예외로 이 라이선스를 따릅니다.
